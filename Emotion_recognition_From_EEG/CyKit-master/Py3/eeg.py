...
#  Modified       by ShuchangHuang
#  Modified       by YuyanZhou
...
# ¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯
folder_path = 'G:\eeg\CyKit-master\Py3\data\ExtractedFeatures'
...
class EEG(object):
    ...
    def paper_svm(self,folder_path):
        print("------------------------")
        '''
        ...
        accuracy = 0
        print("cnmd")
        print(de_LDS_feature_dict.keys())
        for key in de_LDS_feature_dict.keys():
            ...
            # 定义 svm 分类器
            self.svc_classifier = svm.SVC(C=0.8, kernel='rbf')
            self.svc_classifier.fit(train_feature, train_label)
    ...
    def predict(self,emoString):
        emotionList = emoString.split(",")
        emotionList = emotionList[:-2]
        emoDataList = []
        for emotion in emotionList:
            emoDataList.append(float(emotion))
        emoData = [np.array(emoDataList)]
        print(emoData)
        #return
        pred_label = self.svc_classifier.predict(emoData)
        db = MySQLdb.connect("","","")
        cursor = db.cursor()
        try:
            print(emoData[0][0])
            if(emoData[0][0]<100):
                cursor.execute("INSERT INTO arduinodb.eeg_data(eeg)VALUES (%d)"% (pred_label[0]))
                db.commit()
        except Exception:
            print(Exception)
            
       # print(pred_label)#result of eeg
        
        # print(confusion_matrix(test_label, pred_label))
        # print(classification_report(test_label, pred_label))
        #cur_accuracy = self.svc_classifier.score(test_feature, test_label)
        #accuracy += cur_accuracy
        #print('当前 experiment 的 accuracy 为：{}'.format(cur_accuracy))
                ...
                try:
                    ...
                    if self.KeyModel == 6 or self.KeyModel == 5:
                        ...
                        if self.format < 1:
                            ...
                            packet_data = packet_data[:-len(self.delimiter)-2] # Remove extra delimiter.
                            ...
                            if self.outputdata == True:
                               # mirror.text(str( packet_data))#modified by YuyanZHOU
                               # paper_svm(folder_path,packet_data)
                                self.predict(packet_data)
                        #  ~Format-1: (Raw Data Format.) (Data not Decoded)
                        # ¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯
                        if self.format == 1:
                                print("===================================================")
